...
from ssa import add_vita_config
...
def do_flop(cfg):
    ...
    import torch
    data = [{"sentence": None, "image": None, "height": None, "width": None}]
    data[0]["sentence"] = "it is a test expression"
    data[0]["image"] = [torch.randn((3, 384, 600))]
    data[0]['height'] = 384
    data[0]['width'] = 600
    data[0]['dataset_name'] = 'mevis'

    for idx in tqdm.trange(args.num_inputs):  # noqa
...
def do_parameter(cfg):
    ...
    n_parameters = sum(p.numel() for p in model.parameters() if not p.requires_grad)
...
def do_speed(cfg):
    ...
    data = [{"sentence": None, "image": None, "height": None, "width": None}]
    data[0]["sentence"] = "it is a test expression"
    data[0]["image"] = [torch.randn((3, 448, 684))] * 4
    data[0]['height'] = 448
    data[0]['width'] = 684
    data[0]['dataset_name'] = 'mevis'

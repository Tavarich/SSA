...
from .clip_attention import ClipStageAdapter, ClipStageAdapterV2
...
class CLIP(Backbone):
    def __init__(self, cfg, input_shape):
        ...
        self.type = cfg.MODEL.FC_CLIP.TYPE
        ...
        self.stage_adapter_layers = nn.ModuleList()
        for i in range(4):
            if self.type == 'V2':
                self.stage_adapter_layers.append(
                    ClipStageAdapterV2(visual_dim=self.output_channels[i + 1],
                                       language_dim=self.dim_latent,
                                       hidden_dim=256)
                )
            else:
                self.stage_adapter_layers.append(
                    ClipStageAdapter(visual_dim=self.output_channels[i + 1],
                                     language_dim=self.dim_latent,
                                     hidden_dim=256)
                )
    ...
    def extract_features(self, x, sent, word, word_mask, T):
        return {
            'convnext': self.extract_features_convnext,
        }[self.model_type](x, sent, word, word_mask, T)
    ...
    def extract_features_convnext(self, x, sent, word, word_mask, T):
        sent = sent.repeat_interleave(T, dim=0)
        word = word.repeat_interleave(T, dim=0)
        word_mask = word_mask.repeat_interleave(T, dim=0)
        ...
        for i in range(4):
            ...
            if self.type == 'V2':
                x = self.stage_adapter_layers[i](x, sent, word, word_mask)
            else:
                x = self.stage_adapter_layers[i](x, word, word_mask)
        ...
        out['clip_latent'] = self.clip_model.visual.head(x)  # add trunk.head get image level feature
        return out
    ...
    def encode_text_features(self, text, normalize: bool = False):
        ...
        all_x = x @ self.clip_model.text_projection
        all_x = F.normalize(all_x, dim=-1) if normalize else all_x

        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.clip_model.text_projection  # [batch_size, d_model]
        x = F.normalize(x, dim=-1) if normalize else x

        return all_x, x

    def get_text_features(self, text_list, context_length, device):
        with torch.no_grad():
            text_tokens = self.text_tokenizer(text_list)
            text_tokens = text_tokens.to(device)  # zero as padding
            word_feat, sentence_feat = self.encode_text_features(text_tokens, normalize=False)

            word_feat = word_feat[:, :context_length]  # [batch_size, context_length, d_model]
            attention_mask = (text_tokens != 0).bool()
            attention_mask = attention_mask[:, :context_length]  # [batch_size, context_length]

            return word_feat, sentence_feat, attention_mask  # word level, sentence level

    def forward(self, x, sent, word, word_mask, T):
        return self.extract_features(x, sent, word, word_mask, T)

...
class SSAlignment(nn.Module):
    ...
    def __init__(
        ...
        self.save_dir = self.cfg.OUTPUT_DIR
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            # return self.inference_visualize(batched_inputs[0])
    ...
    def get_padded_feat(self, text_feat, N=40):
        ...
        for i, feat in enumerate(text_feat):
            length = min(feat.shape[0], N)
            padded_word_feat[i, :length, :] = feat[:length, :]            
            padded_mask[i, :length] = 1  
        return padded_word_feat, padded_mask.bool()
    ...
    def train_model(self, batched_inputs):
        ...
        padded_word_feat, lang_feat, padded_mask = self.backbone.get_text_features(sentences, 40, self.device)
        lang_feat = lang_feat.unsqueeze(dim=1)
    ...
    def inference(self, batched_inputs):  # only support batch=1
        ...
        padded_word_feat, lang_feat, padded_mask = self.backbone.get_text_features(sentences, 40, self.device)
        lang_feat = lang_feat.unsqueeze(dim=0)
        lang_mask = None

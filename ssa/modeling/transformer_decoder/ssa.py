...
class SSA(nn.Module):
    ...
    def forward(self, frame_query, sentence_feat, lang_mask):
        ...
        frame_query = frame_query.permute(1, 2, 0, 3).contiguous()  # T, fQ, LB, C
        frame_query = self.input_proj_dec(frame_query)  # T, fQ, LB, C

        # NOTE frame query bi-part match
        src = self.src_embed(frame_query)  # T, fQ, LB, C
        for i in range(1, T):
        ...
        time_weight = self.time_aware_attn(src)  # T, fQ, LB, 1
        time_weight = F.softmax(time_weight, 0)  # T, fQ, LB, 1
        output = (src * time_weight).sum(0)  # cQ, LB, C

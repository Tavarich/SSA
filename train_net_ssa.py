...
from ssa.evaluation import inference_on_dataset
...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        print('buding traing dataset')
        mappers = []
        for d_i, dataset_name in enumerate(cfg.DATASETS.TRAIN):
            if dataset_name.startswith('mevis'):
                mappers.append(
                    MeViSDatasetMapper(cfg, is_train=True, is_tgt=True, src_dataset_name=dataset_name)
                )
            elif dataset_name.startswith('ytvos') or dataset_name.startswith('davis'):
                mappers.append(
                    YTVOSDatasetMapper(cfg, is_train=True, is_tgt=True, src_dataset_name=dataset_name)
                )
            elif dataset_name.startswith('refcoco'):
                mappers.append(
                    CocoDatasetMapper(cfg, is_train=True, is_tgt=False, src_dataset_name=dataset_name)
                )
            else:
                raise NotImplementedError
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        if dataset_name.startswith('mevis'):
            ...
            mapper = YTVOSDatasetMapper(cfg, is_train=False, src_dataset_name=dataset_name)
        elif dataset_name.startswith('refcoco'):
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            with torch.amp.autocast('cuda'):
                    output_folder = os.path.join(cfg.OUTPUT_DIR, "inference")
                    results_i = inference_on_dataset(model, data_loader, evaluator, output_folder)
